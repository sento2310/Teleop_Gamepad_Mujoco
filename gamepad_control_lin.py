...
"""
@file gamepad_control_lin.py
@brief Linux-specific launcher for robotic arm teleoperation system
@details Uses Linux-optimized teleoperation modules for XInput and DInput controllers.
         Provides unified entry point with robot selection interface.
"""
...
from genericteleoperation_lin import GenericTeleoperationLin as GenericTeleoperation
from so100teleoperation_lin import SO100TeleoperationLin as SO100Teleoperation
from config import Configuration


class GamepadControlLauncher:
    """
    @brief Main launcher class for robotic arm teleoperation systems (Linux)
    ...
    def select_robot(self, robot_name):
        ...
        robot_config = Configuration.get_robot_config(robot_name)
    ...
    def initialize_teleop_system(self):
        ...
        try:
            if self.selected_robot == 'so100':
                ...
                print("Initialized SO100 teleoperation system (Linux)")
            else:
                self.teleop_system = GenericTeleoperation(robot_name=self.selected_robot)
                print(f"Initialized Generic teleoperation system for {self.selected_robot} (Linux)")
...
def main():
    """
    @brief Main entry point for Linux gamepad control application
    ...
    print("=" * 60)
    print("ROBOTIC ARM TELEOPERATION SYSTEM (LINUX)")
    print("=" * 60)
    ...
    print("\nUsage options:")
    print("  1. Run directly: python gamepad_control_lin.py [robot_name]")
    print("  2. Run interactively: python gamepad_control_lin.py")
    print("=" * 60)
